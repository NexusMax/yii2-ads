...
class CategoriesController extends MainController
{
    ...
    public function actionIndex()
    {
        ...
        if (Yii::$app->request->isAjax) {
            $caterory = Categories::find()->where(['id' => Yii::$app->request->post('id')])->orderBy('sort ASC')->one();
            $caterory->active = Yii::$app->request->post('checkbox_active');
            $caterory->save();
        }

        $categories = Categories::getTree(Categories::getCategories($sort->orders));

        return $this->render('index', compact('categories', 'sort'));
    }
    ...
    public function actionDelete($id)
    {
        ...
        if(!empty($model->image)){
            @unlink(Yii::getAlias('@appWeb') . '/uploads/categories/'. $model->image);
        }
        ...
        if(!empty($child_model)){
           foreach ($child_model as $key) {
                ...
                if(!empty($childer_model->image)){
                    @unlink(Yii::getAlias('@appWeb') . '/uploads/categories/'. $childer_model->image);
                }
