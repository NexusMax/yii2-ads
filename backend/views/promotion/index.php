...
<!--     <p>
    ...
    <?= Html::beginForm('/admin/promotion/index', 'get', ['enctype' => 'multipart/form-data']) ?>
    <?= Html::input('number', 'id', Yii::$app->request->get('id'), ['class' => '', 'placeholder' => 'ID']) ?>
    <?= Html::submitButton('Искать', ['class' => 'submit']) ?>
    <?= Html::endForm() ?>
        ...
        <div class="table-wrap">
            <div class="table-inner-wrap">
                <div class="table-header">
                    ...
                    <div class="header-section header-section-name">Обьявление</div>
                    <div class="header-section header-section-name">Тип продвижения</div>
                    <div class="header-section header-section-name"><?= $sort->link('create'); ?></div>
                    <div class="header-section header-section-name"><?= $sort->link('validity'); ?></div>
                    <div class="header-section header-section-name">Дней</div>
                    <div class="header-section header-section-name">Цена</div>
                    <div class="header-section header-section-name">Пакет</div>
                ...
                <?php foreach ($promotion as $key): ?>
                        ...
                        <div class="table-body">
                            ...
                            <div class="body-section body-section-name"><a href="<?= Url::to(['/ads/update', 'id' => $key['ads_id']]) ?>"><?= $key['ads']['name'] ?></a></div>
                            <div class="body-section body-section-name"><?= $promotionName[$key['type']] ?></div>
                            <div class="body-section body-section-name"><?= Yii::$app->formatter->asDate($key['created_at'], 'php:d/m/Y H:i') ?></div>
                            <div class="body-section body-section-name"><?= Yii::$app->formatter->asDate($key['validity_at'], 'php:d/m/Y H:i') ?></div>
                            <div class="body-section body-section-name"><?= $key['type_time'] ?></div>
                            <div class="body-section body-section-name">

                                <?php if(!empty($key['package'])):  ?>
                                    <?= $key['package']['price'] ?>
                                <?php else: ?>
                                    <?= $key['price'] ?>
                                <?php endif; ?>
                            ...
                            <div class="body-section body-section-name">
                                <?php if(!empty($key['package'])):  ?>
                                    <?= $packageName[$key['package']['promotion']] ?>
                                <?php else: ?>
                                    <?= 'Нет' ?>
                                <?php endif; ?>
