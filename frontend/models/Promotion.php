...
use Yii;
use yii\base\Model;
use frontend\models\AdsHasImage;
...
{	
    ...
    public static function getNameTypes($type = null)
    {
        return [
            2 => 'ВИП',
            3 => 'ТОП',
    ...
    */
    ...
    public function save()
    {
        ...
        if(!empty($this->top_) || !empty($this->vip) || !empty($this->up) || !empty($this->fire) || !empty($this->once_up) || !empty($this->promotion)){
            ...
            if(!empty($this->top_) || !empty($this->vip) || !empty($this->up) || !empty($this->fire) || !empty($this->once_up)){
                ...
                if(!empty($promo_arr)){
                    foreach ($promo_arr as $key) {
                        $this->message['type'][] = ['type' => $key[1], 'days' => $key[4]];
                    }
                }
            ...
            if(!empty($this->promotion)){
                foreach ($this->promotion as $key) {
                    if(!empty($key)){
                        ...
                        $this->message['package'][] = ['type' => $this->getNumberPackege($key), 'days' => ''];
    ...
    public function afterrSave(){
        
        $smsm_message ='';
        $message = '<ul>';
        if(!empty($this->message)){

            foreach ($this->message['package'] as $key) {
                $message .= '<li>Пакет услуг "' . self::getNameTypesPackage()[$key['type']] . '" установлен.</li>';
                $smsm_message .= "Пакет услуг " . self::getNameTypesPackage()[$key['type']] . " установлен. \n";
            }

            foreach ($this->message['type'] as $key) {
                $message .= '<li>Услуга "' . self::getNameTypes()[$key['type']] . '" сроком на ' . $key['days'] . ' дней установлена.</li>';
                $smsm_message .= "Услуга " . self::getNameTypes()[$key['type']] . " сроком на " . $key['days'] . " дней установлена.";
            }
        }
        $message .= '</ul>';
        $this->sms_message = $smsm_message;
        $this->message = $message;

    }

    public static function getNameTypesPackage($type)
    {
        return [
            1 => 'Легкое начало',
            2 => 'Золотая середина',
